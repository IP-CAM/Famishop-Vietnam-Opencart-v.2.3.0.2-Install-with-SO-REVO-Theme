...
class ControllerPagePageForm extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('page/page_form', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('page/page_form/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('page/page_form/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['page_forms'][] = array(
				...
				'edit'       	=> $this->url->link('page/page_form/edit', 'token=' . $this->session->data['token'] . '&page_form_id=' . $result['page_form_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . '&sort=o.title' . $url, true);
		$data['sort_sort_order'] = $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . '&sort=o.sort_order' . $url, true);
		$data['sort_status'] = $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . '&sort=o.status' . $url, true);
		...
		$pagination->url = $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['page_form_id'])) {
			$data['action'] = $this->url->link('page/page_form/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('page/page_form/edit', 'token=' . $this->session->data['token'] . '&page_form_id=' . $this->request->get['page_form_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('page/page_form', 'token=' . $this->session->data['token'] . $url, true);
