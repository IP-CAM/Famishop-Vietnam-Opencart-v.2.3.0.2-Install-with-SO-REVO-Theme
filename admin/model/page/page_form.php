...
class ModelPagePageForm extends Model {
	public function addPageForm($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "page_form` SET show_guest = '" . (int)$data['show_guest'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', customer_email_status = '" . (int)$data['customer_email_status'] . "', admin_email_status = '" . (int)$data['admin_email_status'] . "', top = '" . (isset($data['top']) ? (int)$data['top'] : '') . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : '') . "', captcha = '" . (int)$data['captcha'] . "'");

		$page_form_id = $this->db->getLastId();

		foreach ($data['page_form_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "page_form_description SET page_form_id = '" . (int)$page_form_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', customer_subject = '" . $this->db->escape($value['customer_subject']) . "', customer_message = '" . $this->db->escape($value['customer_message']) . "', admin_subject = '" . $this->db->escape($value['admin_subject']) . "', admin_message = '" . $this->db->escape($value['admin_message']) . "', success_title = '" . $this->db->escape($value['success_title']) . "', success_description = '" . $this->db->escape($value['success_description']) . "', fieldset_title = '" . $this->db->escape($value['fieldset_title']) . "', submit_button = '" . $this->db->escape($value['submit_button']) . "'");
		}
			...
			foreach ($data['page_form_field'] as $page_form_field) {
				...
				if(isset($page_form_field['description'])) {
					foreach ($page_form_field['description'] as $language_id => $page_form_option_description) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "page_form_option_description SET page_form_option_id = '" . (int)$page_form_option_id . "', page_form_id = '" . (int)$page_form_id . "', language_id = '" . (int)$language_id . "', field_name = '" . $this->db->escape($page_form_option_description['field_name']) . "', field_error = '" . $this->db->escape($page_form_option_description['field_error']) . "', field_placeholder = '" . $this->db->escape($page_form_option_description['field_placeholder']) . "'");
					}
				}
	...
	public function getPageForm($page_form_id) {
		$query = $this->db->query("SELECT *, (SELECT DISTINCT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'page_form_id=" . (int)$page_form_id . "') AS keyword  FROM `" . DB_PREFIX . "page_form` o WHERE o.page_form_id = '" . (int)$page_form_id . "'");
