...
function feature_boxYTShortcode($atts,$contentC,$module_id,$id,$database) {
	...
	/*--------------------------------------------------*/
		if($atts->name_shortcode_status == "yes" && $text_name_shortcode != '' && $atts->style != 'style1'){
		...
		if($atts->icon !=''){
			if (strpos($atts->icon, 'icon:') !== false)	{
				...
				$atts_icon = '<img src="'.yt_image_media($atts->icon).'"  alt="" /> ';
			}
		}
		$return .= '<div id="'.$id.'" style="color:'.$atts->box_color.'" class="yt-clearfix yt-feature-box yt-feature-box-style-' . $atts->style .' '.$atts->yt_class.' text-'.$atts->align.'">';

				$return .=  '<div class="yt-feature-box-title"><h3>'. $text_yt_title . '</h3></div>';
				$return .=  '<div class="yt-feature-box-icon"><span>'.$atts_icon.'</span></div>';
				if($atts->name_shortcode_status == "yes" && $text_name_shortcode != '' && $atts->style == 'style1'){
					$return .= '<h3 class="shortcodeTitle">'.$text_name_shortcode.'</h3>';
				}
				$return .=  '<div class="yt-feature-box-content yt-clearfix">'.$text_content.'</div>';
			$return .=  '</div>';
	}
	return $return;
}
