                ...
                <div class="col-sm-10">
                  ...
                  <div id="blog-category" class="well well-sm" style="height: 150px; overflow: auto;">
                    ...
                    <div id="blog-category<?php echo $blog_category['blog_category_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $blog_category['name']; ?>
                      <input type="hidden" name="blog_category[]" value="<?php echo $blog_category['blog_category_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
...
$('input[name=\'category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=bts_blog/category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
...
$('input[name=\'related\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=bts_blog/blog/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
