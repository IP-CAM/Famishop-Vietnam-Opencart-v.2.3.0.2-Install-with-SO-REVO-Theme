...
class ControllerPageSuccess extends Controller {
	public function index() {
		...
		if ($page_form_info) {
			...
			if(VERSION < '2.2.0.0') {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/page/success.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/page/success.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/page/success.tpl', $data));
				}
			} else{
