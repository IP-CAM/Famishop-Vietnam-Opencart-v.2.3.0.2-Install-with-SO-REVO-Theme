...
class ModelPageRequest extends Model {
	public function addPageRequest($data) {
		...
		if($page_form_info) {
			if(!empty($page_form_info['customer_email_status'])) {
				$page_request_info = $this->getPageRequestEmail($page_form_info['page_form_id']);
	...
	public function getPageRequestEmail($page_form_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "page_request_option` WHERE  `page_form_id` = '" . (int)$page_form_id . "' AND (`type` = 'email' OR `type` = 'email_exists')");
