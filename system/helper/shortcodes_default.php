...
class ShortcodesDefault extends Controller
{
    ...
    /**
     * Load module pagebuilder
     *
     * @example [module_pagebuilder id="7" /]
     */
    public function module_pagebuilder($atts)
    {
        extract($this->shortcodes->shortcode_atts(array(
            'id'    => 0,
        ), $atts));
        ...
        if ($id) {
          ...
          $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "module WHERE module_id = '" . (int)$id . "' AND code = 'so_page_builder'");
            $setting = json_decode($query->row['setting'], true);
  

            $module = $this->load->controller('extension/module/so_page_builder', array(
                'moduleid' => $id,
                'page_builder' => $setting['page_builder'],
            ));

            $html = '<div class="shortcode-module sc-page-builder">' . $module . '</div>';
